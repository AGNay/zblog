...
<div class="post">
    <h2><span title="[{$lang['msg']['top']}]" class="istop"></span><a href="{$article.Url}">{$article.Title}</a></h2>
    <div class="info">
        {php}
        $post_info = array(
    ...
    {if tpure_Thumb($article) != ''}<div class="postimg"><a href="{$article.Url}"><img src="{tpure_Thumb($article)}" alt="{$article.Title}"></a></div>{/if}
    ...
    {if $zbp->Config('tpure')->PostMOREBTNON}<div><a href="{$article.Url}" class="readmore">{$lang['tpure']['viewmore']}</a></div>{/if}
</div>
...
function Server_Open($method) {
	...
	case 'down':
		...
		if ($xml === false) {
			...
			$zbp->ShowError('App下载失败！', __FILE__, __LINE__);
		}
		...
		if (App::UnPack($s)) {
			...
			if(property_exists ('App','check_error_count')){
				...
				if($i >0){
					$zbp->SetHint('bad', $i . '个文件写入失败,请检查主机权限配置!');
				}
			}
	...
	case 'view':
		...
		if (strpos($s, APPCENTRE_DOMAIN) === false) {
			$zbp->ShowHint('bad', '客户端访问应用中心故障，不能登录和下载应用。请检查：(1)主机空间是否能远程访问'.APPCENTRE_DOMAIN.'，(2)在设置中切换后台远程连接的方式，(3)在设置中切换应用中心备用访问域名app.'.str_replace(array( str_replace('app.','',APPCENTRE_DOMAIN),'|'), '', APPCENTRE_DOMAINS).'进行访问。');
		}
		$s = str_replace('%bloghost%', $zbp->host . 'zb_users/plugin/AppCentre/main.php', $s);
		echo str_replace('%csrf_token%', $zbp->GetToken('AppCentre'), $s);
		break;
